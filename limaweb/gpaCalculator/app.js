...
//防止form的button交出表單
let allButtons = document.querySelectorAll("button"); //return NodeList因此下面加表格時要再做addEventListener
...
addButton.addEventListener("click", () => {
  ...
  let textNode0 = document.createTextNode("A+");
...
function handleSorting(direction) {
  ...
  //取得array object後，把成績string換成number(用converter)
  ...
  for (let i = 0; i < graders.length; i++) {
    graders[i].children[3].value = objectArray[i].class_grade; //graders.children[3]就是select
  }

  //因為更改後只有改innerHTML沒有eventListener，所以下面要加個物件的eventListener
  //select事件監聽
